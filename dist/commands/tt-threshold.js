    ...
    cy.reload().then(() => {
      cy.window().then((reloadedWin) => {
        ...
        Object.keys(categorizedResources).forEach((type) => {
          categorizedResources[type].forEach((resource) => {
            const sizeKB =
              (resource.transferSize || resource.decodedBodySize) / 1024
        ...
        cy.log(
          `Total page weight: ${pageWeightMB.toFixed(2)} MB (${pageWeightKB.toFixed(2)} KB)`
        )
        // Delay assertion to ensure logs are displayed
        cy.then(() => {
          if (pageWeight > thresholdBytes) {
            assert.fail(
              `Page weight exceeds threshold: ${pageWeightMB.toFixed(2)} MB (${pageWeightKB.toFixed(2)} KB)`
            )
          } else {
            cy.log(
              `Page weight is within the threshold: ${pageWeightMB.toFixed(2)} MB (${pageWeightKB.toFixed(2)} KB)`
            )
          }
        })
      })
