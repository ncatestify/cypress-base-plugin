...
const ttValidateFormLabels = () => {
    ...
    cy.get('input, textarea, select').then(($elements) => {
        ...
        $elements.each((index, element) => {
            ...
            const hasAnyLabel = hasLabel ||
                isWrappedInLabel ||
                hasAriaLabel ||
                hasAriaLabelledby ||
                hasTitle;
            if (!hasAnyLabel) {
                errors.push(`<${tagName}${id ? ` id="${id}"` : ''}${name ? ` name="${name}"` : ''} type="${type}"> has no associated label (element ${index + 1})`);
            }
        });
        if (errors.length > 0) {
            throw new Error(`Form label validation failed (${errors.length} issues):\n${errors.join('\n')}`);
        }
        else {
...
exports.ttValidateFormLabels = ttValidateFormLabels;
